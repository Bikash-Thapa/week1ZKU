...
template sudoku() {
    ...
    component puzzleRange[9][9]; // initializing RangeProof components for puzzle range
    component solutionRange[9][9]; // initializing RangeProof components for puzzle solution range
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {

            // component instantantiation to RangeProof template
            puzzleRange[i][j] = RangeProof(32);
            solutionRange[i][j] = RangeProof(32);

            // Argument passing to RangeProof template for puzzle
            puzzleRange[i][j].in <== puzzle[i][j];
            puzzleRange[i][j].range[0] <== 0;
            puzzleRange[i][j].range[1] <== 9;

            // Argument passing to RangeProof template for puzzle solution
